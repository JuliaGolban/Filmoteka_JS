...
export default function pagination(totalPages, currentPage) {
    ...
    if (window.innerWidth > 320 && window.innerWidth < 767.98) {
        if (currentPage > 1) { markup += `<li class="pagination__left pagination__elem"><svg id="svg__left" class="icon__arrow icon__arrow--left" width="10px" height="10px"><use id="use__left" href="${sprite}#arrow-pag"></use></svg></li>` }
        ...
        if (totalPages > currentPage) { markup += `<li class="pagination__elem pagination__elem--page">${afterPage}</li>` }
        if (totalPages - 1 > currentPage) { markup += `<li class="pagination__elem pagination__elem--page">${afterTwoPage}</li>` }
        ...
        if (currentPage > 1) { markup += `<li class="pagination__left pagination__elem"><svg id="svg__left" class="icon__arrow icon__arrow--left" width="10px" height="10px"><use id="use__left" class="use" href="${sprite}#arrow-pag"></use></svg></li>` }
...
function onPaginationBoxClick(evt) {
    ...
    if (evt.target.nodeName !== 'LI') {
        if (evt.target.nodeName !== 'svg') {
            if (evt.target.nodeName !== 'use') { return };
        }
    };
    if (evt.target.className === 'pagination__left pagination__elem' || evt.target.id === 'svg__left' || evt.target.id === 'use__left') {
        getResponse(globalCurrentPage -= 1, partUrl);
        return;
    };
    if (evt.target.className === 'pagination__right pagination__elem' || evt.target.id === 'svg__right' || evt.target.id === 'use__right') {
        getResponse(globalCurrentPage += 1, partUrl);
        return;
    };
